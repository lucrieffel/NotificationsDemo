...
class CopingActivityViewModel: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    // MARK: - Convert Dates to Firestore Timestamps Without Changing the Local Time

    /// Convert `Date` to `Timestamp` while preserving the local time components via ISO8601 formatting.
    /// This ensures Firestore will display the same local time that iOS sees.
    ...
    func addCopingActivity(activity: CopingActivity) {
        ...
        Task {
            ...
            do {
                ...
                // 7.TODO: Schedule follow-up notification for mood check-in after activity
...
extension CopingActivityViewModel {
